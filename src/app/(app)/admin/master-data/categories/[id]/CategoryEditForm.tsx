...
export default function CategoryEditForm({
  ...
  const defaultValues: CategoryFormValues = {
    ...
    subject_categories:
      initialData.subject_categories?.map((item: any) => ({
  ...
  const onSubmit = async (values: CategoryFormValues) => {
    ...
    try {
      await axiosClient.put(
        `/api/supervisor/categories/${initialData.id}/`,
        payload
      );
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Card 2: Subject List Editor */}
        <SubjectListEditor control={form.control} allSubjects={allSubjects} />

        {/* Footer Actions */}
        <div className="flex justify-end gap-4 pt-4 border-t border-border bg-background/95 backdrop-blur-sm sticky bottom-0 py-4 z-10 rounded-b-lg">
          <Button
            type="button"
            variant="outline"
            onClick={onCancel}
