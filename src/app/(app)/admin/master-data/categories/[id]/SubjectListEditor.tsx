  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-base">Danh sách môn học <span className="text-red-500">*</span></CardTitle>
      </CardHeader>
                      ...
                      <SelectContent>
                        {allSubjects.map((sub) => {
                          ...
                          const isSelected = watchedSubjects?.some(
                            (item, idx) => item.subject_id === subIdStr && idx !== index
                          );
                          ...
                          return (
                            <SelectItem 
                                key={sub.id} 
                                value={subIdStr}
                                disabled={isSelected} // Khóa dòng trùng
                                className={isSelected ? "opacity-50" : ""}
                            >
                              {sub.name} {isSelected ? "(Đã chọn)" : ""}
                            </SelectItem>
                          );
                        })}
                      </SelectContent>
